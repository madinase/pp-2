...
def create_table():
    ...
    if connection is not None: 
        try:
            ...
            """)
            connection.commit() 
            print("Таблица успешно создана/проверена.")
...
def insert_from_csv(file_path): 
    ...
    if connection:
        try:
            ...
            connection.commit()
            print(f'Данные из {file_path} импортированы.')
        except Exception as ex:
...
def update_contact(id, new_name=None, new_phone=None):
    ...
    if conn:
        try:
            ...
            conn.commit()
            print(f'ID {id} обновлен.')
        except Exception as ex:
            print(f'Ошибка обновления: {ex}')
        finally:
            cur.close()
            conn.close()
...
def delete_contact(name=None, phone=None):    
    if not name and not phone:
        print("Ошибка: укажите данные для удаления!")
        return
    conn = connect()
    if conn:
        try:
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            path = input("Введите название файла (например, contacts.csv): ")
            insert_from_csv(path)
        elif choice == '3':
            name_f = input("Фильтр по имени (пусто = все): ")
            phone_f = input("Фильтр по телефону (пусто = все): ")
            get_contact(name_filter=name_f, phone_filter=phone_f)
        elif choice == '4':
            _id = input("Введите id: ")
            get_contact(con_id=_id)
        elif choice == '5':
            _id = input("Введите id контакта: ")
            new_name = input("Новое имя (Enter для пропуска): ")
            new_phone = input("Новый телефон (Enter для пропуска): ")
            update_contact(_id, new_name=new_name, new_phone=new_phone)
        elif choice == '6':
            name = input("Имя для удаления (Enter для пропуска): ")
            phone = input("Телефон для удаления (Enter для пропуска): ")
